...
num_revolutions = 2; % Number of spiral motions
radius = 5; % Radius of the circular trajectory in the x-y plane
z_increment_per_revolution = 5; % Vertical distance moved up per revolution
total_time = 30; % Total time for the trajectory
t = linspace(0, total_time, 300); % Time vector

% Initialize trajectory vectors
x_drone_des = zeros(1, length(t));
y_drone_des = zeros(1, length(t));
z_drone_des = zeros(1, length(t));

% Define parameters for the circular trajectory
omega = 2 * pi * num_revolutions / total_time; % Angular velocity for circular motion

% Calculate the desired trajectory
for i = 1:length(t)
    % Time in the current spiral phase
    time_in_spiral = mod(t(i), total_time / num_revolutions);
    revolution_fraction = time_in_spiral / (total_time / num_revolutions);
    
    % Calculate the z position
    z_drone_des(i) = 1 + z_increment_per_revolution * floor(t(i) / (total_time / num_revolutions)) + revolution_fraction * z_increment_per_revolution;
    
    % Calculate the circular motion in the x-y plane
    angle = omega * t(i);
    x_drone_des(i) = radius * cos(angle);
    y_drone_des(i) = radius * sin(angle);
end
...
for sim = 1:num_simulations
    ...
    transient_time_indices = find(t <= 5);
    steady_state_time_indices = find(t > 5);
...
% Plot histogram of Transient RMS errors (0-5 seconds)
figure('Position', [100, 100, 800, 600]);
histogram(rms_errors_transient, 'Normalization', 'pdf', 'BinWidth', 0.01);
title('Histogram of Transient RMS Errors (0-5 seconds)', 'FontSize', 14);
...
% Plot histogram of Steady State RMS errors (5-30 seconds)
figure('Position', [100, 100, 800, 600]);
histogram(rms_errors_steady_state, 'Normalization', 'pdf', 'BinWidth', 0.01);
title('Histogram of Steady State RMS Errors (5-30 seconds)', 'FontSize', 14);
xlabel('RMS Error (m)', 'FontSize', 12);
ylabel('Frequency', 'FontSize', 12);
grid on;
